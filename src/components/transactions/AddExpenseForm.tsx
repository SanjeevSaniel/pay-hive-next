...
const AddExpenseForm = () => {
  ...
  const handleAmountPaidChange = (userId: string, amount: number) => {
    ...
    // TODO: Implement percentage and custom split methods
    if (selectedSplitMethod === SplitMethod.Percentage) {
      const updatedDetails = splitDetails.map((detail) => {
        const percentageAmount = ((detail.amount || 0) / 100) * totalPaid;
        return {
          ...detail,
          amount: percentageAmount - (updatedAmounts[detail.userId] || 0),
        };
      });
        ...
        const totalPercentage = splitDetails.reduce((sum, detail) => {
          const percentage =
            ((detail.amount + (amountPaidByPayees[detail.userId] || 0)) /
              totalPaid) *
            100;
          // const percentage = (detail.amount / totalPaid) * 100;
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    //   fromUserId: 'user1',
    //   toUserId: 'user2',
    //   originalExpenseIds: ['expense1', 'expense2'],
    //   // ... other fields
    // };
